...
Type EXPresolve_op_and( Expression e, Scope s ) {
    fprintf( stderr, "DEBUG: EXPresolve_op_and called\n" );
    ...
    if( refinements ) {
        Refinement r = refinements;
        fprintf( stderr, "DEBUG: Collected refinements:\n" );
        while( r ) {
            fprintf( stderr, "  - Variable: %s, Type: %s\n",
                    r->variable->name->symbol.name ? r->variable->name->symbol.name : "(unnamed)",
                    r->refined_type->symbol.name ? r->refined_type->symbol.name : "(unnamed)" );
            r = r->next;
        }
    } else {
        fprintf( stderr, "DEBUG: No refinements collected\n" );
    }
    ...
    fprintf( stderr, "DEBUG: Resolving RHS with refinements active\n" );
